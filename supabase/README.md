...
**云端是唯一数据源，本机只是它的镜像。** 这条约定决定了下面所有行为：

### 拉取：以云端为准，全量对齐

每次打开页面、切换项目、点「立即拉取云端数据」时，本机会与云端**全量对齐**，包括：

- 云端新增的记录 → 拉到本机
- 云端修改的记录 → 覆盖本机（含修订次数 revision）
- **云端已删除的记录 → 从本机移除**
- **云端已删除的项目 → 从本机移除**

> 为什么必须这样：早期版本使用「只增不删」的合并方式，导致别人删掉的记录会残留在你本机；
> 更严重的是，你下次保存任何记录时，那条残留记录会被**重新写回云端**（已删数据"复活"）。
> 现版本已改为全量对齐，并有专门的测试覆盖（`test/sync-multi-user.test.js`）。

### 写入：先云端，成功后才写本地

保存记录、删除记录、删除项目都是**先操作云端**：

- 云端成功 → 再更新本机
- 云端失败 → 报错并**不改本机**

> 好处：本机永远不会存在「云端没有」的记录，从根上杜绝复活问题。
> 代价：**断网时无法保存**（会明确提示失败），这是为了保证一致性而做的取舍。

### 冲突与并发（实测结论）

| 情形 | 实际行为 | 是否安全 |
|---|---|---|
| 两人同时新增**不同周期** | 各自成功，互不影响 | ✅ |
| 两人同时写**同一周期** | 先到者成功；后到者被拒（`NOT_OWNER`），云端只留一条 | ✅ 数据安全，但后到者会看到失败提示 |
| 用**陈旧副本**覆盖别人的新数据 | 被服务端拒绝，新数据保留 | ✅ |
| 同一人并发写同一周期 | 数据库唯一约束保证只留一条 | ✅ |
| 断网时保存 | 明确报错，不写本地 | ⚠️ 需联网 |

**「同一周期被他人占用」的处理**：工具的提示是——请换一个周期名，或让对方自行修改。
如果你希望同周期也能多人协作（例如一人一条子记录），需要改动数据模型，目前不支持。

### 已知限制

1. **没有实时推送**：别人新增的数据要等你下次拉取（打开页面/切换项目）才出现，不是秒级同步。若需要边看边自动更新，可加轮询（当前未实现）。
2. **会话丢失的后果**：记录归属由浏览器的匿名会话决定。若你清除了浏览器数据，**自己以前提交的记录将无法再修订或删除**（别人也不行）。处理办法见下方「孤儿数据」一节。
3. **免费层的暂停**：项目连续 7 天无访问会被暂停，登录控制台恢复即可，数据不丢。
4. **数据量**：当前拉取是「一次拉全部项目」。实测 1000 条约 2.4 MB、约 8 秒（3G），可接受；
   若记录数上万，应改为按项目增量拉取。
