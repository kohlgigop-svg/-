-- =============================================================================
-- 补丁 2：修复 qc_fetch_all 的字段引用错误 + 统一所有函数的 search_path
-- -----------------------------------------------------------------------------
-- 修复内容：
--   1. qc_fetch_all 里排序写成 t.name，但子查询输出的字段名是 project_name
--      → 报 column t.name does not exist（42703）。已改为 t.project_name。
--   2. 6 个业务函数的 search_path 只写了 public，而 Supabase 把 pgcrypto
--      装在 extensions schema，存在解析不到的风险，且会被安全审查标记。
--      → 全部统一为 public, extensions。
--
-- 使用：Supabase 控制台 → SQL Editor → New query → 粘贴本文件全文 → Run
--       应显示 Success. No rows returned
--
-- 说明：本补丁只重建函数，不动表结构与已有数据，安全可重复执行。
-- =============================================================================
...
-- =============================================================================
-- 验证：在控制台执行（把 qc-eval-2026 换成你实际使用的访问码）
--   select public.qc_fetch_all('qc-eval-2026');   -- 应为 []（或已有的项目数组）
-- =============================================================================
